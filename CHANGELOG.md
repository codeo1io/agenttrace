...
- TUI test wait loops (`wait_for_pending_load`, `wait_for_progress`, and the governance-delivery loop) now bound every wait by an explicit deadline instead of a fixed iteration count, eliminating load-dependent flakes on slow or contended runners; a failing wait names its deadline rather than a loop count (fork PR #3).
- Oh My Pi session files that open with a non-session line (typically a title record) now parse instead of failing the whole file with `oh_my_pi: missing session header` — the parser skips leading non-session objects before requiring the session header, matching upstream PR #284; a file with no session header anywhere still declines the Oh My Pi path and falls back to the generic parser.
- The last non-saturating token-account arithmetic in the parser now saturates: usage-map accumulation, token-total deltas (a provider total reset no longer produces negative deltas), and the Codex uncached-input/output computations. A session whose token fields are `i64::MAX` parses without panicking in debug builds; totals pin at the ceiling instead of wrapping.
- `--no-baseline-gate` no longer swallows the flag that follows it: the Go-compatible argument shim misclassified the boolean flag as a value flag, so `agenttrace --no-baseline-gate --overview` silently consumed `--overview` as its value. The shim's value-flag table is now pinned against clap's own definitions by a contract test.
- Workspace publish metadata stays pinned to the upstream original: the planned repoint to the fork remote (CU-28) was reverted before merge because `scripts/ci/check-cargo-manifests.sh` (inherited from upstream) requires `repository`/`homepage` to name `luoyuctl/agenttrace`. Whether fork crates should ever carry fork metadata is an open steward policy question, not a silent edit to an inherited guard.
- The `--sample` disclosure names the active view (`--sort`/`--order`) it sampled in, out-of-scope data-health counting is per source unit rather than per session, `--sample` outside governance-class actions is rejected with a named error, the text compare path carries the same sampled-exclusion reason as JSON, and headerless session-cache entries are evictable under the cache bounds (cycle-5 review findings F5-1..F5-5, landed with this cycle after their filing gap).
