...
- `install.sh` now verifies the downloaded release asset against the `.sha256` sidecar the release workflow already publishes (pass-11 A11-3): a present sidecar that mismatches aborts before anything is executed, naming expected and actual hashes; an absent sidecar (older releases) or a missing sha256 tool warns and continues. The installed binary is forced to mode 0755 before the move, so a restrictive `umask` (verified under `077`) can no longer yield a group/other-unreadable binary the way `mktemp`'s 0600 plus `chmod +x` did.
