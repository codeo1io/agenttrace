use agenttrace_core::{
    ...
    DataHealth, LoadOptions, Session, TraceStep,
};
use serde::Serialize;
...
#[derive(Default)]
struct AppState {
    sessions: Mutex<Option<Arc<Vec<Session>>>>,
}

impl AppState {
    fn get_or_load(
        &self,
        load: impl FnOnce() -> anyhow::Result<Vec<Session>>,
    ) -> Result<Arc<Vec<Session>>, String> {
        let mut sessions = self.sessions.lock().map_err(|error| error.to_string())?;
        if sessions.is_none() {
            *sessions = Some(Arc::new(load().map_err(|error| error.to_string())?));
        }
        Ok(sessions.clone().unwrap_or_default())
    }

    fn replace(&self, sessions: Vec<Session>) -> Result<(), String> {
        *self.sessions.lock().map_err(|error| error.to_string())? = Some(Arc::new(sessions));
        Ok(())
    }
}
...
fn load_sessions() -> anyhow::Result<Vec<Session>> {
    let mut seen = HashSet::new();
    Ok(load_sessions_with_options(None, &LoadOptions::default())
        .sessions
        .into_iter()
        .filter(|session| {
            seen.insert((
...
fn sessions(state: &State<'_, AppState>) -> Result<Arc<Vec<Session>>, String> {
...
fn display_source(source: &str) -> String {
    match source {
        ...
        "hermes" | "hermes_agent" => "Hermes".to_string(),
...
fn load_home(state: State<'_, AppState>) -> anyhow::Result<HomeDto, String> {
    ...
    Ok(HomeDto {
        ...
        data_health: data_health(&sessions, total_sessions, 0),
    })
}
...
mod tests {
    ...
    fn desktop_state_loads_once_until_explicit_refresh() {
        ...
        state.replace(Vec::new()).unwrap();
        assert!(state.get_or_load(|| unreachable!()).unwrap().is_empty());
    }
