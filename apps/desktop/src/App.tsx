...
import {
  AlertTriangle, ArrowDown, ArrowRight, BarChart3, Bot, CalendarDays, Check, CheckCircle2, ChevronDown,
  ChevronRight, CircleDollarSign, Clock3, Coins, FolderOpen, Gauge, Home as HomeIcon, Languages,
...
export function App() {
  ...
  async function loadAll(refresh = false) {
    ...
    try {
      ...
      setSelectedId((current) => current || sessionData.sessions[0]?.id || "");
  ...
  useEffect(() => { if (selectedId) void api.getSession(selectedId).then(setDetail).catch(() => setDetail(null)); }, [selectedId]);
  ...
  return <div className="app-shell">
    <Sidebar view={view} navigate={navigate} t={t} />
...
function Sidebar({ view, navigate, t }: { view: View; navigate: (view: View) => void; t: Copy }) {
  ...
  return <aside className="sidebar">
    ...
    <div className="local-state"><span className="status-dot" />{t.systemHealthy}</div>
  </aside>;
}
...
function Home({ sessions, findings, language, t, openSessions, openDiscover, refresh }: { sessions: SessionSummary[]; findings: Finding[]; language: Language; t: Copy; openSessions: () => void; openDiscover: () => void; refresh: () => void }) {
  ...
  return <div className="page home-page">
    ...
    <section className="metric-grid">{metrics.map(([label,value,Icon,tone]) => <article className="metric-card" key={label}><div><small>{label}</small><strong>{value}</strong><span className="metric-change">↓ {language === "zh" ? "较昨日" : "vs yesterday"}</span></div><span className={`metric-icon ${tone}`}><Icon /></span></article>)}</section>
    <section className="home-grid">
      <article className="chart-card panel"><div className="section-heading"><h2>{t.healthTrend}</h2><span>{t.healthScore}</span></div><div className="trend-chart" aria-label={t.healthTrend}><svg viewBox="0 0 800 240" preserveAspectRatio="none"><defs><linearGradient id="area" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#39b96e" stopOpacity=".22"/><stop offset="1" stopColor="#39b96e" stopOpacity="0"/></linearGradient></defs><path className="chart-area" d="M0 130 C80 80 115 180 180 128 S290 155 350 104 S470 148 540 91 S680 120 800 72 L800 240 L0 240Z"/><path className="chart-line" d="M0 130 C80 80 115 180 180 128 S290 155 350 104 S470 148 540 91 S680 120 800 72"/></svg><span className="axis top">100%</span><span className="axis bottom">0%</span></div><div className="chart-legend"><span><i className="green-dot" />{t.healthySessions} {Math.round(averageHealth)}%</span><span><i className="orange-dot" />{t.needsAttention} {attentionRate}%</span></div></article>
...
function Sessions({ sessions, selectedId, select, detail, language, t, refresh }: { sessions: SessionSummary[]; selectedId: string; select: (id:string)=>void; detail: SessionDetail|null; language: Language; t: Copy; refresh:()=>void }) {
  ...
  return <div className="page sessions-page"><PageHeader title={t.sessions}/><div className="sessions-toolbar"><label className="search-box"><Search/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder={t.search}/></label><div className="filter-buttons">{[["all",t.all],["smooth",t.smooth],["attention",t.needsAttention]].map(([id,label])=><button key={id} className={filter===id?`active ${id}`:""} onClick={()=>setFilter(id)}>{label}</button>)}<button aria-label={t.refresh} onClick={refresh}><RefreshCw/></button></div></div>
    ...
    {modal&&<SessionModal detail={detail} language={language} t={t} close={()=>setModal(false)}/>}</div>;
}
